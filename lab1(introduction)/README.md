...
В раммках первого занятия вам предстоит познакомится с основами языка C++ (напишем и разберём подробно первую программу), а также с системой контроля версий **Git**, с которой мы будем работать на протяжении семестра.

## Организационные моменты

Каждая лабораторная работа находится в своей директоии в репозитории (например, эта находится в папке **lab1(introductioin)**). 
...
Выполнять лаботаторную работу можно прямо в этой же папке, т.е. можно создавать/удалять файлы, редактировать файлы (кроме файлов с заданиями!), если иное не сказано в задании. После чего вы загружаете свои исходные файлы в свой репозиторий(см. "Задания к лабораторной работе").

Для сдачи лабораторной работы недостаточно лишь залить её на github. Также нужно защитить работу преподавателю лично(это делается на занятиях).

Фрагменты кода на занятиях я буду демонстрировать в теминале linux (у меня стоит Ubuntu 24.04.3) используя текстовый редактор Vim(также иногда буду использовать Visual Studio Code). Также иногда буду использовать WSL (Windows Subsystem for Linux) с версией Ubuntu 20.04 (при использовании операционной системы Windows). 

В вашем случае для написания кода вы можете использовать, что вам удобнее — будь то текстовый редактор или какая угодно среда разработки (например CLion или Microsoft Visual Studio).
...
В рамках предмета мы будем работать с одним репозиторием [cpp_labs](). Каждому студенту нужно скопировать этот репозиторий на свой аккаунт github (так назыавемый **fork**), с которым он будет работать и загружать результаты своей проделанной работы. Подробнее как это сделать будет разобрано на занятии.
...
> Периодически репозиторий будет обновляться, поэтому перед выполнением лабораторной работы необходимо выполнять следующие 3 команды из вашего локального репозитория:
```
git stash
git pull
git stash pop
...
Добавить файл **main.cpp** и файл(ы) с решением лабораторной работы в вашу ветку (если вы работаете напрямую с веткой main, то 4 пункт выполнять не нужно). Это можно сделать, например, так:
...
3. загружаем эти изменения в вашей ветке из вашего локального репозитория на удаленный (в нашем случае на Git):
    ...
    ```
4. выполняем "слияние" нашей ветку с основной веткой main. Это можно сделать тоже по-разному:
    1. прямо на сайте github в своём репозитории. Вам сайт предложит создать pull request и затем слить ветки (рекомендую делать так);
    2. с помощью команд терминала:
        - переходим на основную ветку (в нашем случае это ветка main), с которой мы хотим слить нашу:
            ```
            git checkout main
            ```
        - сливаем основную ветку с нашей (иногда могут случаться конфликты):
            ```
            git merge "название ветки, которую хотим слить с main" 
            ```
        - удаляем нашу ветку (не ветку main!!!!)
            ```
